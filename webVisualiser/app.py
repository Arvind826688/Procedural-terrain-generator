...
# Permutation array for gradient calculation
PERMUTATION = [
...
def perlin_noise(x, y, seed):
    ...
    aa = PERMUTATION[PERMUTATION[x0] + y0 + seed % 256]
    ab = PERMUTATION[PERMUTATION[x0] + y1 + seed % 256]
    ba = PERMUTATION[PERMUTATION[x1] + y0 + seed % 256]
    bb = PERMUTATION[PERMUTATION[x1] + y1 + seed % 256]

    grad_aa = gradient(aa, dx, dy)
    grad_ba = gradient(ba, dx - 1, dy)
    grad_ab = gradient(ab, dx, dy - 1)
    grad_bb = gradient(bb, dx - 1, dy - 1)

    u = fade(dx)
    v = fade(dy)

    lerp_x1 = lerp(u, grad_aa, grad_ba)
    lerp_x2 = lerp(u, grad_ab, grad_bb)

    return lerp(v, lerp_x1, lerp_x2)
...
def generate_height_map():
    ...
    height_map = np.zeros((breadth, length), dtype=np.uint16)
    for y in range(breadth):
        for x in range(length):
            noise_value = perlin_noise(x, y, seed)
            noise_value = (noise_value + 1) / 2  # Normalize [0, 1]
            height_map[y, x] = int(noise_value * (height - 1))

    return jsonify(height_map.tolist())
